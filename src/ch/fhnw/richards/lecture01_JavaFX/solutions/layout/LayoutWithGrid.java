package ch.fhnw.richards.lecture01_JavaFX.solutions.layout;
...
public class LayoutWithGrid extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		// We need spacers, because GridPane
		// automatically collapses empty rows and columns
		...
		// Set which column grows on resizing
		for (int i = 0; i <=3; i++) {
			ColumnConstraints cc = new ColumnConstraints();
			if (i == 2) cc.setHgrow(Priority.ALWAYS);
			root.getColumnConstraints().add(cc);
		}
		
		// Set which row grows on resizing
		for (int i = 0; i <= 6; i++) {
			RowConstraints rc = new RowConstraints();
			if (i == 5) rc.setVgrow(Priority.ALWAYS);
